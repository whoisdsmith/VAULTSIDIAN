...
This image will appear when [[IFTTT|[[IFT]]TT|[[IFT]]TT|[[IFT]]TT]] is set up to post an image or a file, but we don't get the data as expected.
...
Note that this code is specific to the Tumblr > Twitter Applet mentioned above. If you'd like to use this filter code in your Applet, `Tumblr.newAnyPost.PostImageUrl` and `Twitter.postNewTweetWithImage.skip()` will need to be replaced with your trigger's ImageURL ingredient and your action[[IFTTT|[[IFT]]TT|'s sk]]ip code, respectively.
