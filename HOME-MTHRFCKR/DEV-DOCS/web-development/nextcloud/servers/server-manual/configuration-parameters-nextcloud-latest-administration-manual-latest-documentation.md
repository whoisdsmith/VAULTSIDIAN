...
The salt used to hash all passwords, auto-generated by the Nextcloud installer. (There are also per-user salts.) If you lose this salt you lose all your passwords. This example is for documentation only, and you should never use it.
    ...
    To have clean URLs without /index.phpnone this parameter needs to be configured.
    
    
    
    
    This parameter will be written as “RewriteBase” on update and installation of
    Nextcloud to your .htaccessnone file. While this value is often simply the URL
    ...
    After setting this value run occ maintenance:update:htaccessnone. Now, when the
    following conditions are met Nextcloud URLs won’t contain index.php:
      ...
      * mod_rewritenone is installed
    
    
    
      * mod_envnone is installed
    ...
    For server setups, that don’t have mod_envnone enabled or restricted (e.g. suEXEC)
    this parameter has to be set to true and will assume mod_rewrite.
    
    
    
    
    Please check, if mod_rewritenone is active and functional before setting this
    parameter and you updated your .htaccess with occ maintenance:update:htaccessnone.
    ...
    rules. Please refer to [Controlling file versions and aging](../configuration_files/file_versioning.html) for
    more information.
...
>     
    ...
    Supported conditions:
      ...
      * 
    shared_secret: if a request parameter with the name log_secretnone is set to
...
>     
    ...
    - IPv4 addresses, e.g. 192.168.2.123none
    - IPv4 ranges in CIDR notation, e.g. 192.168.2.0/24none
    ...
    When an incoming request’s REMOTE_ADDRnone matches any of the IP addresses
    specified here, it is assumed to be a proxy instead of a client. Thus, the
    client IP will be read from the HTTP header specified in
    forwarded_for_headersnone instead of from REMOTE_ADDR.
    
    
    
    
    So if you configure trusted_proxiesnone, also consider setting
    forwarded_for_headersnone which otherwise defaults to HTTP_X_FORWARDED_FOR
    ...
    trusted_proxiesnone. If the HTTP header looks like ‘X-Forwarded-For’, then use
    ...
    is highly recommended to _avoid data loss_.
    ...
    Before enabling this, read the warning in [Activities in groupfolders or external storages](activity_configuration.html#label-activities-groupfolders)
