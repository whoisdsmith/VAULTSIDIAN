...
If your HTTP server is configured to use a different PHP version than the default (/usr/bin/php), `occ` should be run with the same version. For example, in CentOS 6.5 with SCL-PHP70 installed, the command looks like this:
    ...
    See [Background jobs](background_jobs_configuration.html) to learn more.
    ...
    destinationuid. You can use the force option -fnone to enforce the move if there
    ...
    See [Calendar / CalDAV](../groupware/calendar.html) for more information on how to use this command.
    ...
    See [Encryption configuration](../configuration_files/encryption_configuration.html) to learn more.
    ...
    See [user documentation](https://docs.nextcloud.com/server/latest/user_manual/en/files/transfer_ownership.html) for details.
    ...
    See [Code signing](../issues/code_signing.html) to learn more.
    ...
    documented in [LDAP user cleanup](../configuration_user/user_auth_ldap_cleanup.html).
    ...
    **users** and **db-admins** groups. Any groups that do not exist are created:
    ...
    [How to upgrade](../maintenance/upgrade.html)) use this command to upgrade your databases,
