...
Memcaches must be explicitly configured in Nextcloud by installing and enabling your desired cache, and then adding the appropriate entry to `config.php` (See [Configuration Parameters](config_sample_php_parameters.html) for an overview of all possible config parameters).

You may use both a local and a distributed cache. Recommended caches are APCu and Redis. After installing and enabling your chosen memcache, verify that it is active by running [PHP version and information](../issues/general_troubleshooting.html#label-phpinfo).
...
After restarting your Web server, add this line to your `config.php` file:
    ...
    as a key-value store for [Transactional File Locking](../configuration_files/files_locking_transactional.html) because it guarantees
    ...
    does not package Redis at all, see Additional Redis installation help.
    ...
    Redis is very configurable; consult [the Redis documentation](http://redis.io/documentation) to learn more.
    ...
    **Using the Redis session handler:** If you are using Redis for locking and/or caching,
    ...
    filesnone handler. If you use the Redis handler, though, you _MUST_ ensure that session
    locking is enabled. As of this writing, the Redis session handler does _NOT_ enable
    ...
    [PhpRedis GitHub page](https://github.com/phpredis/phpredis)
    ...
    **Connecting to Redis over TLS:**
    ...
    with [Transactional File Locking](../configuration_files/files_locking_transactional.html)
    ...
    (See [Configuration Parameters](config_sample_php_parameters.html)) to select a different location.
    ...
    php-redis, then try [this Redis guide on Tech and Me](https://www.techandme.se/install-redis-cache-on-ubuntu-server-with-php-7-and-nextcloud/) for a complete Redis installation on Ubuntu 14.04 using PECL.
