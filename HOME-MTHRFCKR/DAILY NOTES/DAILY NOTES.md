%% Begin Waypoint %%
- **[DAILY NOTES](./DAILY%20NOTES.md)**
	- [2022-11-04](./2022-11-04.md)
	- [2022-11-07](./2022-11-07.md)
	- [2022-11-08](./2022-11-08.md)
	- [2022-11-10](./2022-11-10.md)
	- [2022-11-11](./2022-11-11.md)
	- [2022-11-12](./2022-11-12.md)
	- [DAILY NOTES](./DAILY%20NOTES.md)
	- [mthrfckr-dot-tech](./mthrfckr-dot-tech.md)
	- [Search operators you can use with Gmail](./Search%20operators%20you%20can%20use%20with%20Gmail.md)
	- [SSTweets](./SSTweets.md)

%% End Waypoint %%

Misc
eyedex - open directory search engine
ODCrawler
Leet - Open Directory Search
Mamont's open FTP Index
The Best Design Tools Apps and Products of 2022 | Product Hunt
Find All You Need For AI Art | aiart[apps]
Pinegraph | Create AI art with Stable Diffusion, Disco Diffusion, Dall E 2, and Midjourney
🤖 🖼 AI Art Generator, AI Art Maker - NightCafe Creator
Start Page
Playground AI
Ai generative art tools
Maks-s/sd-akashic: A compendium of informations regarding Stable Diffusion (SD)
Craiyon, formerly DALL-E mini
Generative Engine | RunwayML Experiments
FAQ
invoke-ai/InvokeAI: This version of Stable Diffusion features a slick WebGUI, an interactive command-line script that combines text2img and img2img functionality in a "dream bot" style interface, and multiple features and other enhancements. For more info, see the website link below.
Manual Installation, macOS - Stable Diffusion Toolkit Docs
Download
Insight
Siri Shortcuts - ShortcutsGallery.com
Welcome to Nicegram | Nicegram
Spacedrive — A file manager from the future.
enzobes/WarezBot: Public Version of Discord bot for scene release
Free Indexers
What is Usenet? Complete Guide to Usenet & How to Use It in 2022
Multi-OCH Helper
Microsoft Privacy Statement – Microsoft privacy
API ― Raindrop.io Help
Overview - API Documentation
Import
help.raindrop.io
MobileFirstLLC/social-media-hacker-list: Growing list of apps and tools for enhancing social media experiences.
