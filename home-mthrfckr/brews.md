...
- **apfelstrudel:** A tool for generating random data for testing and benchmarking purposes.
- **appium:** An open-source test automation framework for use with native, hybrid, and mobile web apps.
- **apr:** Apache Portable Runtime, a library providing a predictable and consistent interface to underlying platform-specific implementations.
- **apr-util:** A utility library for APR (Apache Portable Runtime) providing additional functionality like XML parsing, database access, and more.
- **argon2:** A password-hashing function that provides a good balance between memory and computational complexity.
...
- **autogen:** A tool designed for generating program files that contain repetitive text with varied substitutions.
- **automake:** A tool for automatically generating Makefile.in files from files called Makefile.am.
...
- **byobu:** A terminal multiplexer and window manager that allows multiple sessions to be created and managed from a single terminal window.
...
- **dav1d:** A fast and small AV1 decoder.
- **deeptools:** A package for analyzing high-throughput sequencing data, including ChIP-seq, RNA-seq, and ATAC-seq.
- **dintch:** A tool for creating and managing desktop shortcuts on macOS.
...
- **easyengine:** A command-line tool for managing WordPress sites on a server.
...
- **fluid-synth:** A software synthesizer that can play MIDI files using SoundFont technology.
- **fmt:** A text
- **fontconfig:** A library for configuring and customizing font access.
- **font-hack-nerd-font:** A patched version of the Hack font that includes additional glyphs and icons for use in programming and terminal applications.
- **fortune:** A command-line tool that displays random quotes and sayings.
- **fping:** A tool for pinging multiple hosts at once and reporting the response times.
- **freetds:** A library for connecting to Microsoft SQL Server databases from Unix and Linux systems.
...
- **gas-mask:** A macOS hosts file manager that allows users to switch between multiple hosts files.
- **gcc:** The GNU Compiler Collection, which includes front ends for C, C++, Objective-C, Fortran, Ada, and others.
- **gd:** A graphics library for the dynamic creation of images.
- **gdbm:** A library for creating and managing persistent key-value databases in C.
- **gdk-pixbuf:** A library for loading and manipulating images in various formats, used by GNOME and other Linux desktop environments.
- **gettext:** An internationalization (i18n) and localization (l10n) system commonly used for writing multilingual programs on Unix-like computer systems.
- **gh:** The official command-line interface for GitHub, which allows users to interact with repositories, pull requests, and other GitHub features.
...
- **gnu-sed:** A version of the sed text editor that includes GNU extensions and can handle larger files.
- **gnutls:** A modern and easy-to-use software library for encryption, decryption, and authentication.
- **gnutls:** A modern and easy-to-use software library implementing the Transport Layer Security (TLS) protocol and Datagram Transport Layer Security (DTLS) protocol.
...
- **grails:** A web application framework based on the Groovy programming language.
- **graphite2:** A font rendering and shaping library that provides support for complex scripts and languages.
- **graphviz:** Open-source graph visualization software.
- **grip:** A command-line tool that allows you to preview your markdown files as HTML in your web browser.
- **groovy:** An agile and dynamic language for the Java Virtual Machine (JVM), offering both static and dynamic typing, native integration with Java code, and powerful scripting capabilities.
- **gts:** A library for manipulating and rendering 3D triangulated surfaces.
- **guile:** A programming language and library for creating and embedding scripting languages.
...
- **hub:** A command-line tool that provides GitHub integration for Git.
- **hwloc:** A library for detecting and manipulating hardware topology (e.g., CPUs, caches, memory) in parallel computing systems.

## I

- **icu4c:** International Components for Unicode (ICU) is a mature, widely used set of C/C++ and Java libraries providing Unicode and Globalization support for software applications.
- **imagemagick:** A software suite to create, edit, compose, or convert bitmap images.
- **imath:** A library for arbitrary precision integer and floating-point arithmetic.
- **imlib2:** An advanced replacement library for libraries like libXpm that provides many more features with much greater flexibility and speed.
- **influxdb-cli:** A command-line interface for InfluxDB, a time-series database.
- **ipatool:** A command-line tool for manipulating IPA (iOS App Store Package) files.
...
- **libass:** A library for rendering subtitles in video files.
- **libassuan:** A library that provides a simple IPC (inter-process communication) system for GnuPG and related software.
- **libavif:** A library for encoding and decoding AVIF images.
- **libb2:** A library that provides fast and secure hash functions.
- **libbluray:** A library for reading and decrypting Blu-ray discs.
- **libcbor:** A library for encoding and decoding data in the Concise Binary Object Representation (CBOR) format.
- **libevent:** An asynchronous event notification software library, providing a mechanism to execute a callback function when a specific event occurs on a file descriptor or after a timeout has been reached.
- **libffi:** A portable, high-level programming interface to various calling conventions, allowing a programmer to call any function specified by a call interface description at runtime.
- **libfido2:** A library for implementing the FIDO2 authentication standard.
- **libgcrypt:** A general-purpose cryptographic library based on the code from GnuPG.
- **libgit2:** A portable, pure C implementation of the Git core methods, providing a linkable library with a solid API, allowing you to write native-speed custom Git applications in any language that supports C bindings.
- **libgpg-error:** A library that provides error codes and descriptions for GnuPG-related software.
- **libiconv:** A library for character encoding conversion.
- **libid3tag:** A library for reading and writing ID3 tags in MP3 audio files.
- **libidn2:** A library for handling internationalized domain names (IDN).
- **libimagequant:** A library for high-quality color quantization of images.
- **libksba:** A library that provides X.509 and CMS (Cryptographic Message Syntax) support for GnuPG and related software.
- **liblinear:** A library for large-scale linear classification and regression tasks.
- **libmaxminddb:** A library for working with MaxMind DB files, which are databases containing geolocation information.
- **libmpc:** A library for performing arithmetic with complex numbers.
- **libmpdclient:** A library for connecting to and controlling MPD (Music Player Daemon) servers.
- **libnfs:** A library for working with the NFS (Network File System) protocol.
...
- **libpthread-stubs:** A library for providing stub functions for the pthreads API.
- **libraqm:** A library for complex text layout, specifically for bidirectional and script shaping.
- **librist:** A library for implementing the RIST (Reliable Internet Stream Transport) protocol for low-latency video transport over unreliable networks.
- **librsvg:** A library for rendering SVG (Scalable Vector Graphics) images.
- **libsamplerate:** A library for sample rate conversion of audio signals.
- **libscrypt:** A library for key derivation using the scrypt algorithm.
- **libshout:** A library for streaming audio over the internet.
- **libsndfile:** A library for reading and writing audio files in various formats.
- **libsodium:** A library for encryption, decryption, signatures, password hashing, and more.
- **libsoxr:** A library for high-quality sample rate conversion of audio signals.
- **libssh2:** A library implementing the SSH2 protocol.
- **libtasn1:** A library for handling Abstract Syntax Notation One (ASN.1) data structures.
- **libtermkey:** A library for processing keyboard input in a terminal.
- **libtiff:** A library for reading and writing TIFF, a widely used format for storing raster images.
- **libtiff:** A library that provides support for the Tag Image File Format (TIFF), a widely used format for storing image data.
- **libtool:** A generic library support script, part of the GNU build system, simplifying the process of creating portable compiled libraries.
- **libunibreak:** A library for line breaking and word breaking algorithms that work on Unicode text.
- **libunistring:** A library for manipulating Unicode strings.
- **libupnp:** A library for implementing the UPnP (Universal Plug and Play) protocol.
- **libusb:** A library for interacting with USB devices.
- **libuv:** A library for asynchronous I/O operations.
- **libvidstab:** A library for video stabilization.
- **libvmaf:** A library for computing the Video Multi-Method Assessment Fusion (VMAF) metric, which is used to measure video quality.
- **libvorbis:** A library for working with the Vorbis audio codec.
- **libvpx:** A video codec library for encoding and decoding VP8 and VP9 video formats.
- **libvterm:** A library for implementing a VT220/xterm terminal emulator.
- **libwebsockets:** A library for implementing the WebSocket protocol.
- **libx11:** A library for working with the X Window System.
- **libxau:** A library for handling authentication in the X Window System.
- **libxcb:** A library for interfacing with the X Window System protocol at the lowest level.
- **libxdmcp:** A library for handling the X Display Manager Control Protocol.
- **libxext:** A library for working with X Window System extensions.
- **libxml2:** A software library for parsing XML documents, providing a full implementation of the XML 1.0 and XML 1.1 specifications, as well as a large number of associated standards (XPath, XPointer, XInclude, etc.).
- **libxrender:** A library for working with X Window System rendering.
...
- **lychee:** A program for managing photos and albums.
- **lz4:** A lossless data compression algorithm that is focused on compression and decompression speed, providing extremely fast compression and decompression with a reasonable compression ratio.
- **lzo:** A library for data compression and decompression using the L

## M

- **m4:** A macro processing language used to generate text files, such as code or configuration files.
...
- **mbedtls:** A lightweight, modular, and efficient library implementing cryptographic algorithms and protocols.
...
- **mpdecimal:** A library for performing decimal floating point arithmetic.
- **mpfr:** A library for performing floating-point arithmetic with arbitrary precision.
- **mpg123:** A command-line music player that supports various audio formats.
- **msgpack:** A binary serialization format for exchanging data between different languages and platforms.
- **mtr:** A command-line tool that combines the functionality of traceroute and ping for network diagnostic purposes.
- **multimarkdown:** A version of Markdown that supports additional features such as footnotes, tables, and metadata.
- **mutt:** A text-based email client with support for multiple accounts and advanced configuration options.
- **mysql:** An open-source relational database management system.
- **mysql-client:** A command-line client for interacting with MySQL databases.
- **mytop:** A console-based tool for monitoring MySQL server performance in real-time.
...
- **nettle:** A library for performing cryptographic operations such as encryption and hashing.
- **newrelic-infra-agent:** A monitoring agent for the New Relic Infrastructure monitoring platform.
- **newt:** A library for creating text-based user interfaces in a terminal.
...
- **npth:** A library for implementing the OpenPGP smart card protocol.
- **nspr:** A library for providing platform-independent APIs for system-level and network programming.
- **nss:** A library for implementing cryptographic protocols such as SSL/TLS and PKI.
- **ntfs-3g-mac:** A driver for macOS that provides read-write access to NTFS-formatted partitions.
- **nushell:** A modern, cross-platform shell that emphasizes simplicity and productivity.
- **nvm:** A command-line tool for managing multiple versions of Node.js on a single machine.

## O

- **ocaml:** A programming language and environment for functional and imperative programming.
- **odrive:** A client for the odrive cloud storage platform that allows for syncing and managing files across multiple services.
- **oniguruma:** A regular expression library that supports various syntaxes and encodings.
- **opencore-amr:** A library for encoding and decoding Adaptive Multi-Rate (AMR) audio files.
- **openexr:** A high dynamic range (HDR) image file format and associated software library.
- **openjdk@11:** A specific version of OpenJDK 11 that can be installed alongside other versions of Java.
- **openldap:** An open-source implementation of the Lightweight Directory Access Protocol (LDAP) for managing network directories.
- **open-mpi:** An implementation of the Message Passing Interface (MPI) for parallel computing.
...
- **p11-kit:** Provides a way to load and enumerate PKCS#11 modules, which are used for cryptographic operations.
- **pandoc:** A universal document converter, capable of converting files from one markup format to another, supporting a wide variety of input and output formats.
...
- **pinentry:** A collection of tools that allow secure input of sensitive information such as passwords, passphrases, and encryption keys.
- **pixman:** A low-level library for manipulating pixel buffers.
- **pkg-config:** A helper tool used when compiling software that helps find and configure the libraries and headers needed to build the software.
- **podofyllin:** A library used to generate PDF files from various input formats.
- **poppler:** A PDF rendering library used to view, render, and manipulate PDF files.
- **popt:** A command-line option parsing library used by many software projects.
- **portaudio:** A cross-platform audio API used to play and record sound.
- **postgresql:** A powerful, enterprise-class, object-relational database system with over 30 years of active development, providing a strong reputation for reliability, data integrity, and correctness.
- **pratique:** A command-line tool for creating and managing virtual environments in Python.
- **precize:** A command-line tool for performing precise floating-point arithmetic.
- **protobuf:** Google's language-neutral, platform-neutral, extensible mechanism for serializing structured data.
- **proxychains-ng:** A tool used to tunnel network connections through one or more proxy servers.
- **pssh:** A tool used to execute commands on multiple remote machines at the same time.
- **pygments:** A syntax highlighting package written in Python that supports over 500 programming languages.
...
- **rsync:** A utility used to synchronize files and directories between two different locations.
- **rtmpdump:** A toolkit for RTMP streams, used to download or stream online multimedia content.
- **rubberband:** A library used to time-stretch and pitch-shift audio files.
- **ruby:** A dynamic, open-source programming language with a focus on simplicity and productivity, featuring an elegant syntax that is natural to read and easy to write.
- **rust:** A systems programming language that aims to provide memory safety, concurrency, and performance, focusing on safety, speed, and concurrency, and featuring a strong static type system, ownership, and a fearless concurrency model.
...
- **screenresolution:** A command-line tool for changing the screen resolution.
- **scrub:** A tool used to securely erase data from a storage device.
- **sdl2:** A cross-platform development library for creating graphical user interfaces and games.
- **searchkey:** A command-line tool for searching for GPG keys on a keyserver.
- **shfmt:** A shell script formatter that helps to make shell scripts more readable and consistent.
- **signet:** A command-line tool for signing and verifying files using digital signatures.
- **six:** A Python 2 and 3 compatibility library.
- **sl:** A humorous program that displays a steam locomotive animation when the "ls" command is mistyped as "sl".
- **s-lang:** A library for creating multi-platform software with a consistent user interface.
- **sloth:** A tool for profiling Python code and identifying performance bottlenecks.
- **slurm:** A job scheduler and workload manager used in high-performance computing environments.
- **snappy:** A fast compression and decompression library developed by Google.
- **source-highlight:** A library and command-line tool for syntax highlighting source code.
- **speedtest-cli:** A command-line tool for testing internet speed.
- **speex:** A free and open-source audio codec designed for speech compression.
- **sqlite:** A C-language library that implements a small, fast, self-contained, high-reliability, full-featured, SQL database engine, often used as a serverless, zero-configuration, transactional SQL
- **srt:** A command-line program to manipulate subtitle files in the SRT format.
- **s-search:** A command-line tool for searching the web using the DuckDuckGo search engine.
...
- **toilet:** A program that generates ASCII art text-based graphics that can be used for various purposes.
- **tokyo-cabinet:** A library of data structures and utilities for managing key-value databases.
...
- **tuc:** A tool for translating C code into highly optimized ARM machine code.
- **txt2tags:** A lightweight markup language for creating various types of documents such as web pages, slideshows, and more.

## U

- **unbound:** A validating, recursive, and caching DNS resolver that supports DNSSEC and IPv6.
- **uncolored:** A command-line utility that removes color codes from output, making it easier to read and process.
- **unibilium:** A terminfo parsing library that provides advanced support for handling terminal capabilities and attributes.
...
- **vhs:** A tool for viewing and editing binary files in a hex editor-style interface.
...
- **w3m:** A text-based web browser that supports rendering of images and tables, mouse support, and other advanced features.
- **webp:** A modern image format providing superior lossless and lossy compression for images on the web.
- **weechat:** A fast and highly customizable chat client that supports various protocols such as IRC and XMPP.
- **wget:** A free utility for non-interactive download of files from the Web, supporting HTTP, HTTPS, and FTP protocols.
- **wxwidgets:** A cross-platform GUI toolkit for creating native applications

## X

- **x264:** A free software library and application for encoding video streams into the H.264/MPEG-4 AVC format.
- **x265:** A library for encoding video into the High Efficiency Video Coding (HEVC/H.265) video compression format.
- **xattred:** A command-line utility that displays and modifies extended attributes of files and directories on macOS.
- **xorgproto:** A package of X Window System protocol headers that are required for building X11 applications.
- **xvid:** A video codec library that provides high compression ratios with good quality for digital video.
- **xxhash:** A fast and efficient hash algorithm that can be used for checksumming and data integrity checking.
- **xz:** A general-purpose data compression tool with a high compression ratio.
...
- **zstd:** A fast lossless compression algorithm, targeting real-time compression scenarios at zlib-level and better compression ratios.
- **zzz:** A command-line utility that puts your Mac to sleep immediately, bypassing any sleep settings or timers that may be in effect.
