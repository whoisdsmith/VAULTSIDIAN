...
* To get only entries with min/max count, bit of [a[awk](Gnu_Awk.md)ic would help
...
* See also how the above three cases can be done [usi[using grep alone](Gnu_Grep.md#search-strings-from-file) * **Note** input files do not need to be sorted for `grep` solution
...
* Use [seq[seq](home-mthrfckr/Text%20Processing/Command-Line-Text-Processing/Miscellaneous.md#seq)ut if negative numbers, floating point, etc are needed
