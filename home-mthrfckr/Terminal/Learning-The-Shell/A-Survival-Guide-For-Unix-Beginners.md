...
(There are also *hard* links, in which there are two true, indistingushible references to the same file, but these are less commonly used.)
...
To ssh into another computer, use `ssh _user_@_address_` where *user* is the user name of your account on the other machine, and *address* is host name or IP address of the other computer.
